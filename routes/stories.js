...
router.get('/', (req, res) => {
  if (req.query.search) {
    const sql = `SELECT s.id, s.title, s.story, s.image, u.username, s.created_at
      FROM stories s
      INNER JOIN users u
      ON s.user_id = u.id
    ...
    if (err) {
      ...
      db.query(sql, formData, (err, results, fields) => {
        if (err) {
          ...
          const sql2 = `SELECT stories.id AS storyId, stories.title, stories.story, stories.image, users.username, stories.created_at
            FROM stories
            INNER JOIN users
