...
router.get("/", (req, res) => {
  ...
  if (req.query.search) {
    const sql =
      "SELECT s.id, s.title, s.story, u.username, s.created_at FROM stories s INNER JOIN users u ON s.user_id = u.id WHERE (s.title LIKE ? OR s.story LIKE ? OR u.username LIKE ?)";
    db.query(
      sql,
      [
        `%${req.query.search}%`,
...
router.post("/", isLoggedIn, (req, res) => {
  ...
  db.query(sql, formData, (err, results, fields) => {
    if (err) {
      ...
      const sql2 =
        "SELECT stories.id AS storyId, stories.title, stories.story, users.username, stories.created_at FROM stories INNER JOIN users ON stories.user_id = users.id WHERE stories.id = ?";
